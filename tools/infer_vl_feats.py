...
def setup(config_file):
    ...
    cfg.MODEL.META_ARCHITECTURE="OimClipSimpleBiVisionLanguageGTInfer"
...
def save_inference(cfg, model, data_loader, dataset_name,writer):
    ...
    for batch_inputs in data_loader:
        i,l,v = model(batch_inputs)
        pids.append(i.clone().cpu())
        l_feats.append(l.clone().cpu())
        v_feats.append(v.clone().cpu())
    ...
    rst=[pids,l_feats,v_feats]
    torch.save(rst,"{}/{}_vl_feats.pth".format(cfg.OUTPUT_DIR,dataset_name))
