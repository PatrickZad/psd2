...
class Visualizer:
        ...
        *,
        ...
        Args:
            ...
            masks (masks-like object): Supported types are:
                ...
                * :class:`psd2.structures.PolygonMasks`,
                  :class:`psd2.structures.BitMasks`.
    ...
    def draw_rectangle(self, box_coord, alpha=0.5, color="g", line_style="-"):
        """
        Args:
            box_coord (tuple): a tuple containing x0, y0, x1, y1 coordinates, where x0 and y0
                are the coordinates of the image's top left corner. x1 and y1 are the
...
def mlvl_pca_feat(feats, K=1, solver="auto", whiten=True, norm=True):
    """
    feats multi-level bached feature maps
    """
    sizes = []  # (B,C,H,W) of each lvl
    lens = []
    fl_feats = []
    for b_feats in feats:
        bl, cl, hl, wl = b_feats.shape
        sizes.append(b_feats.shape)
        lens.append(bl * hl * wl)
        fl_feat = b_feats.clone().detach().cpu().permute(0, 2, 3, 1).reshape(-1, cl)
        fl_feats.append(fl_feat)
    all_feats = torch.cat(fl_feats, dim=0).numpy()
    pca = PCA(
        n_components=3 * K,
        svd_solver=solver,
        whiten=whiten,
    ...
    feats = list(torch.split(pca_feats, lens))
    for li, si in enumerate(sizes):
        bl, cl, hl, wl = si
        ft = feats[li].reshape(bl, hl, wl, K * 3).permute(0, 3, 1, 2)
        ft = [ft[:, k : k + 3] for k in range(0, ft.shape[1], 3)]
        if norm:
            ft = [(x - x.min()) / (x.max() - x.min()) for x in ft]
            # rescale to [0-255] for visulization
            ft = [x * 255.0 for x in ft]
        feats[li] = ft[0] if K == 1 else ft
    return feats
...
def tsne_embs(embs, labels):
    ...
    embs_arr = embs.cpu().numpy()
    labels_arr = labels.cpu().numpy()
    tsne = manifold.TSNE(n_components=2, init="pca")
    embs_tsne = tsne.fit_transform(embs_arr)
    data_pd = pd.DataFrame(
        {
            "x": embs_tsne[:, 0],
            "y": embs_tsne[:, 1],
            "id": [str(pid) for pid in labels_arr.tolist()],
        }
    )
    fig = plt.figure()
    sns.scatterplot(x="x", y="y", hue="id", data=data_pd)
    buf = io.BytesIO()
...
def domain_hist_img(d_vals, nbins, ignore=0.1):
    """
    vals: num_domains length list, numpy as element
    """

    def _domain_hist(scores, idx):
        import matplotlib.pyplot as plt
        import seaborn as sns
        import io
        import cv2
        ...
        d_types = ["d{}".format(idx)] * len(scores)
    ...
    hist_imgs = []
    for di, dvs in enumerate(d_vals):
        ig_t = ignore
        while dvs[dvs > ig_t].shape[0] == 0:
            ig_t /= 2
        scores = dvs[dvs > ig_t].tolist()
        hist_imgs.append(_domain_hist(scores, di))
    return np.concatenate(hist_imgs, axis=1)
