...
def _load_coco_person_fkp(dataset_dir, subset="train", allow_crowd=True):
    from pycocotools.coco import COCO

    assert subset in subsets
    json_file = opj(dataset_dir, "annotations", "person_keypoints_{}2017.json".format(subset))
    img_root = opj(dataset_dir, "{}2017".format(subset))
    timer = Timer()
    json_file = PathManager.get_local_path(json_file)
    with contextlib.redirect_stdout(io.StringIO()):
    ...
    for img_id in img_ids:
        ...
        for ann_dict in org_ann:
            if ann_dict["category_id"] == 1:  # person
                if ann_dict["num_keypoints"]<nk_threshold:
                    continue
                if allow_crowd:
                    cur_anns.append(ann_dict)
                elif ann_dict["iscrowd"] == 0:
...
def load_coco_ch_50(common_dir, subset="train", allow_crowd=True):
    coco_dicts = _load_coco_person(opj(common_dir, "coco"), subset, allow_crowd)
    ch_dicts = _load_crowd_human(opj(common_dir, "crowd_human"), subset, allow_crowd)
    cocoidx=npr.permutation(len(coco_dicts)).tolist()
    coco_dicts=[coco_dicts[i] for i in cocoidx[:int(len(coco_dicts)*0.5)]]
    chidx=npr.permutation(len(ch_dicts)).tolist()
    coco_dicts=[ch_dicts[i] for i in chidx[:int(len(ch_dicts)*0.5)]]
    return ch_dicts + coco_dicts

def load_coco_ch_75(common_dir, subset="train", allow_crowd=True):
    coco_dicts = _load_coco_person(opj(common_dir, "coco"), subset, allow_crowd)
    ch_dicts = _load_crowd_human(opj(common_dir, "crowd_human"), subset, allow_crowd)
    cocoidx=npr.permutation(len(coco_dicts)).tolist()
    coco_dicts=[coco_dicts[i] for i in cocoidx[:int(len(coco_dicts)*0.75)]]
    chidx=npr.permutation(len(ch_dicts)).tolist()
