...
def _load_coco_person(dataset_dir, subset="train", allow_crowd=True,filter_esmall=False):
    from pycocotools.coco import COCO

    assert subset in subsets
    json_file = opj(dataset_dir, "sama_annotations", "sama_coco_{}.json".format(subset))
    ...
    for img_dict, anno_dict_list in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            if filter_esmall:
                if obj["bbox"][2] * obj["bbox"][3] < esmall_size:
                    continue
...
def load_samacoco_ch(common_dir, subset="train", allow_crowd=True,filter_esmall=False):
    coco_dicts = _load_coco_person(opj(common_dir, "coco"), subset, allow_crowd,filter_esmall)
    ch_dicts = _load_crowd_human(opj(common_dir, "crowd_human"), subset, allow_crowd,filter_esmall)
    return ch_dicts + coco_dicts


def load_samacoco_p(common_dir, subset="train", allow_crowd=True,filter_esmall=False):
    coco_dicts = _load_coco_person(opj(common_dir, "coco"), subset, allow_crowd,filter_esmall)
    return coco_dicts
