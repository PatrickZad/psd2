    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            if self.prompt_location == "prepend":
                # expand relative_position_bias
                mask = torch.cat(
                    (torch.zeros(nW, _H, len_pk, device=attn.device), mask), dim=-1
                )
            attn = attn.view(B // nW, nW, self.num_heads, N, -1) + mask.unsqueeze(
                1
            ).unsqueeze(0)
...
class SidePrefixPromptedSwinTransformer(
    SideSwinTransformer, PrefixPromptedSwinTransformer
):
    def __init__(self, side_start_stage, *args, **kws):
