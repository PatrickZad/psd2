...
def prompt_attention_pool(x_prompt,pool:nn.Conv2d,norm=None):
    ...
    x_prompt=x_prompt.reshape(B*N,C,1,1)
    ...
    x_prompt=tF.conv2d(x_prompt,weight=pool.weight,bias=pool.bias,stride=pool.stride,padding=(0,0),dilation=pool.dilation,groups=pool.groups)
    x_prompt=x_prompt.reshape(B,N,C)
    if norm:
        x_prompt=norm(x_prompt)
    return x_prompt

def kernel_prompt_attention_pool(x_prompt,pool:nn.Conv2d,norm=None):
    # use duplicate padding to be compatible
    B,N,C=x_prompt.shape
    kh,kw=pool.kernel_size
    x_prompt=x_prompt.transpose(1,2).reshape(B,C,-1,kh,kw).permute(0,2,1,3,4).flatten(0,1)
    x_prompt=tF.conv2d(x_prompt,weight=pool.weight,bias=pool.bias,stride=pool.stride,padding=(0,0),dilation=pool.dilation,groups=pool.groups)
    x_prompt=x_prompt.reshape(B,-1,C)
    if norm:
        x_prompt=norm(x_prompt)
    return x_prompt
...
class SelfPromptedMultiScaleAttention(MultiScaleAttention):
    def forward(self, x,prompts):
        if prompts is None:
            return super().forward(x)
        B, H, W, _ = x.shape
        ...
        vp = prompt_attention_pool(vp, self.pool_v, self.norm_v)# Bh * L * c
...
class KernelPromptedMultiScaleAttention(MultiScaleAttention):
    def forward(self, x,prompts):
        if prompts is None:
            return super().forward(x)
        B, H, W, _ = x.shape
        ...
        kp = kernel_prompt_attention_pool(kp, self.pool_k, self.norm_k)
        vp = kernel_prompt_attention_pool(vp, self.pool_v, self.norm_v)# Bh * N * c
        L=kp.shape[1]
...
class PromptedMultiScaleBlock(nn.Module):
    ...
    def forward(self, x, prompts):
        ...
        return x
class SelfPromptedMultiScaleBlock(nn.Module):
    """Multiscale Transformer blocks"""
    ...
    def __init__(
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = SelfPromptedMultiScaleAttention(
            dim,
            dim_out,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
    ...
    def forward(self, x, prompts):
        ...
        pc=prompts.shape[-1]
        if pc>xc:
            #NOTE consider in-stage channel expansion
            prompts=prompts[...,:xc]
...
class KernelPromptedMultiScaleBlock(nn.Module):
    """Multiscale Transformer blocks"""
    ...
    def __init__(
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = KernelPromptedMultiScaleAttention(
            dim,
            dim_out,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
...
class SelfPromptedMViT(nn.Module):
    def __init__(
        self,
        img_size=224,
        patch_kernel=(7, 7),
        ...
        for i in range(depth):
            ...
            # hybrid window attention: global attention in last three stages.
            window_size_ = 0 if i in last_block_indexes[1:] else window_size
            block = SelfPromptedMultiScaleBlock(
                dim=embed_dim,
                dim_out=dim_out,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
...
class KernelPromptedMViT(nn.Module):
    def __init__(
        self,
        img_size=224,
        patch_kernel=(7, 7),
        ...
        for i in range(depth):
            ...
            # hybrid window attention: global attention in last three stages.
            window_size_ = 0 if i in last_block_indexes[1:] else window_size
            block = KernelPromptedMultiScaleBlock(
                dim=embed_dim,
                dim_out=dim_out,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
...
class SidePromptedMViT(SideMViT, PromptedMViT):
    def __init__(self, side_start_stage, *args, **kws):
        ...
        self.side_start_stage = side_start_stage
        self.init_side()
class SideSelfPromptedMViT(SideMViT, SelfPromptedMViT):
    def __init__(self, side_start_stage, *args, **kws):
        SelfPromptedMViT.__init__(self, *args, **kws)
        self.side_start_stage = side_start_stage
        self.init_side()
class SideKernelPromptedMViT(SideMViT, KernelPromptedMViT):
    def __init__(self, side_start_stage, *args, **kws):
        KernelPromptedMViT.__init__(self, *args, **kws)
        self.side_start_stage = side_start_stage
        self.init_side()
class SidePromptedMViTLS(SideMViT, PromptedMViT):
    def __init__(self, side_start_stage, *args, **kws):
        ...
        self.init_side()
class SideSelfPromptedMViTLS(SideMViT, PromptedMViT):
    def __init__(self, side_start_stage, *args, **kws):
        PromptedMViT.__init__(self, *args, **kws)
        self.side_start_stage = side_start_stage
        self.blocks[self._last_block_indexes[-2]+1].attn.pool_q.stride = (1,1)
...
class SidePromptedMViTDC(SideMViT, PromptedMViT):
    def __init__(self, side_start_stage, *args, **kws):
        ...
        if hasattr(self.blocks[self._last_block_indexes[-2]+1],"pool_skip"):
                self.blocks[self._last_block_indexes[-2]+1].pool_skip=nn.Identity()
        self.init_side()
class SideSelfPromptedMViTDC(SideMViT, PromptedMViT):
    def __init__(self, side_start_stage, *args, **kws):
        PromptedMViT.__init__(self, *args, **kws)
        self.side_start_stage = side_start_stage
        self.blocks[self._last_block_indexes[-2]+1].attn.pool_q.stride = (1,1)
