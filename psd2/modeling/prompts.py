...
class CodaPromptWd(nn.Module):
    def __init__(
        ...
        for e in self.e_layers:
            ...
            for ti in range(n_tasks):
                ...
                if ti==0:
                    setattr(self, f"e_p_l{e}_t{ti}", torch.nn.Parameter(p[s:f],requires_grad=True))
                    setattr(self, f"e_k_l{e}_t{ti}", torch.nn.Parameter(k[s:f],requires_grad=True))
                    setattr(self, f"e_a_l{e}_t{ti}", torch.nn.Parameter(a[s:f],requires_grad=True))
                else:
                    setattr(self, f"e_p_l{e}_t{ti}", torch.nn.Parameter(p[s:f],requires_grad=False))
                    setattr(self, f"e_k_l{e}_t{ti}", torch.nn.Parameter(k[s:f],requires_grad=False))
                    setattr(self, f"e_a_l{e}_t{ti}", torch.nn.Parameter(a[s:f],requires_grad=False))
    ...
    def forward(self, x_query, vis_mark, train=False, task_id=None):
        ...
        for l in range(nL):
            ...
            if train:
                if self.task_count > 0:
                    K = torch.cat([getattr(self, f"e_k_l{l}_t{ti}").clone() for ti in range(self.task_count)]+[getattr(self, f"e_k_l{l}_t{self.task_count}")],  dim=0)
                    A = torch.cat([getattr(self, f"e_a_l{l}_t{ti}").clone() for ti in range(self.task_count)]+[getattr(self, f"e_a_l{l}_t{self.task_count}") ],  dim=0)
                    p = torch.cat([getattr(self, f"e_p_l{l}_t{ti}").clone() for ti in range(self.task_count)]+[getattr(self, f"e_p_l{l}_t{self.task_count}") ],  dim=0)
                else:
                    K = getattr(self, f"e_k_l{l}_t0")
                    A = getattr(self, f"e_a_l{l}_t0")
                    p = getattr(self, f"e_p_l{l}_t0")
            else:
                K = torch.cat([getattr(self, f"e_k_l{l}_t{ti}") for ti in range(self.task_count+1)],  dim=0)
                A = torch.cat([getattr(self, f"e_a_l{l}_t{ti}") for ti in range(self.task_count+1)],  dim=0)
                p = torch.cat([getattr(self, f"e_p_l{l}_t{ti}") for ti in range(self.task_count+1)],  dim=0)
...
class DualPrompt(nn.Module):
    def __init__(self, emb_d, n_tasks, e_pool_size,
        num_e_prompts,
        num_g_prompts,
        e_layers,
        g_layers,
    ...
    def _init_smart(self, e_pool_size,num_e_prompts,num_g_prompts,e_layers,g_layers):
            ...
            if l in self.e_layers:
                ...
                if train:
                    ...
                    if self.task_id_bootstrap:
                        loss = (1.0 - cos_sim[:, self.task_count*self.top_k:(self.task_count+1)*self.top_k]).sum()
...
class L2Ppp(DualPrompt):
    def __init__(
            ...
            vis_period,
            key_dim=768):
        super().__init__(emb_d, n_tasks, pool_size,
        num_prompts,
        -1,
...
class L2PppMask(L2Ppp):
    ...
    def forward(self, x_query,vis_mark, train=False, task_id=None):
        if self.training:
            p_list,ploss=super(L2Ppp,self).forward(x_query,vis_mark, train, task_id)
        ...
        for l in range(nL):
            ...
            if l in self.e_layers:
                ...
                if train:
                    ...
                    if self.task_count>0:
                        # negtive loss
                        prev_keys_n=nn.functional.normalize(K[:self.task_count*self.top_k].clone().detach(),dim=1)
                        cur_keys_n=nn.functional.normalize(K[self.task_count*self.top_k:(self.task_count+1)*self.top_k],dim=1)
