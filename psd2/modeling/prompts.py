...
class CodaPrompt(nn.Module):
    def __init__(self, emb_d, n_tasks,pool_size,num_prompts,num_layers,loss_weight,vis_period, key_dim=768):
        super().__init__()
        self.task_count = 0
        self.emb_d = emb_d
        self.key_d = key_dim
...
# }
class L2P(nn.Module):
    def __init__(self, emb_d, n_tasks, pool_size,num_prompts,num_layers,topk,loss_weight,vis_period, key_dim=768):
        super().__init__()
        self.task_count = 0
        self.emb_d = emb_d
        self.key_d = key_dim
    ...
    def forward(self, x_query, vis_mark,train=False, task_id=None):
        ...
        idx_offset=k_idx.new_tensor([i*self.e_pool_size for i in range(nL)]).view(1,-1,1) # offset afetr flatten
        flattened_k_idx=(k_idx+idx_offset).flatten(1) # batch_size, num_layers*topk
        P_=self.e_p.flatten(0,1)[flattened_k_idx] # batch_size, num_layers*topk, e_p_length, emb_d
        ...
        if train:
            if self.vis_period > 0:
                ...
                if storage.iter % self.vis_period == 0:
                    ...
                    vis_img=torch.tensor(vis_img,dtype=torch.float32).permute(2, 0, 1)/255.0
                    storage.put_image(f"batch/prompt_{vis_mark}", vis_img)
            selected_K=self.e_k.flatten(0,1)[flattened_k_idx].reshape(B,nL,self.top_k,C) # batch_size, num_layers, topk, qdim
            batched_selected_sim=torch.einsum('blj,blkj->blk', q, nn.functional.normalize(selected_K, dim=-1))
