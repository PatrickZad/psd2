...
from psd2.modeling.extend.solider import SideSwinTransformer,SidePromptedSwinTransformer,SidePrefixPromptedSwinTransformer,SwinTransformer
...
class SwinF4RCNN(SearchBase):
    ...
    def __init__(
        ...
        **kws,
        ...
        for n,p in self.swin.named_parameters(): # norm2 is not supervised in solider
            if "side_stages.0.downsample" not in n and "side_stages.1" not in n and  "side_semantic_embed_w.0" not in n and  "side_semantic_embed_b.0" not in n and  "side_semantic_embed_w.1" not in n and  "side_semantic_embed_b.1" not in n and not n.startswith("side_norm3") and not n.startswith("side_norm2"): 
                p.requires_grad=False
    ...
    def from_config(cls, cfg):
        ...
        swin=SideSwinTransformer(side_start_stage=3,semantic_weight=tr_cfg.SEMANTIC_WEIGHT,
...
class SwinF4RCNN2(SwinF4RCNN):
    ...
    def from_config(cls, cfg):
        ...
        swin=SideSwinTransformer(side_start_stage=3,semantic_weight=tr_cfg.SEMANTIC_WEIGHT,
            ...
            with_cp=tr_cfg.WITH_CP)
        swin_out_shape={"stage{}".format(i+1): ShapeSpec(channels=swin.num_features[i],stride=swin.strides[i]) for i in range(len(swin.stages))}
        swin_out_norm_shape={"stage{}_norm".format(i+1): ShapeSpec(channels=swin.num_features[i],stride=swin.strides[i]) for i in range(len(swin.stages))}
...
class SwinF4RCNN4(SwinF4RCNN2):
    ...
    @classmethod
    def from_config(cls, cfg):
        assert "norm" in cfg.MODEL.RPN.IN_FEATURES[-1], "Invalid MODEL.RPN.IN_FEATURES config"
    ...
    def swin_backbone(self,x):
        ...
        for i, stage in enumerate(self.swin.stages[:bonenum]):
            ...
            if i == bonenum-1:
                ...
                out_norm = out_norm.view(-1, *out_hw_shape,
                               self.swin.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous()
...
class SwinF4RCNNPS(SwinF4RCNN):
    ...
    def reid_head(self,image_list,backbone_features,roi_boxes,roi_ids=None):
        ...
        if self.training:
            roi_boxes=[Boxes(roi_boxes[i][roi_ids[i]>-2],box_mode=BoxMode.XYXY_ABS) for i in range(len(roi_ids))]
            pos_ids=torch.cat([roi_ids[i][roi_ids[i]>-2] for i in range(len(roi_ids))],dim=0)
        else:
            roi_boxes=[Boxes(roi_boxes_i,box_mode=BoxMode.XYXY_ABS) for roi_boxes_i in roi_boxes]
    ...
    def forward_gallery(self, image_list, gt_instances):
        ...
        if self.training:
            ...
            score_t = self.roi_heads.box_predictor.test_score_thresh
            iou_t = self.roi_heads.box_predictor.test_nms_thresh
            for pred_i,feats_i, gt_i in zip(
                pred_instances,reid_feats, gt_instances
            ):
                pred_boxes_t = pred_i.pred_boxes.tensor
                pred_scores = pred_i.pred_scores
                filter_mask = pred_scores >= score_t
...
class PromptedSwinF4RCNNPS(SwinF4RCNNPS):
    ...
    def task_query(self,backbone_features):
        ...
        out = out.view(-1, *out_hw_shape,
                               self.swin_org.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous()
    ...
    def swin_backbone(self,x):
        ...
        for i, stage in enumerate(self.swin.stages[:bonenum]):
            ...
            selected_prompts,p_loss=self.stage_prompts[i](task_query_stage,i,train=self.training)
            prompt_loss+=p_loss
            x, hw_shape, out, out_hw_shape = stage(x, hw_shape,deep_prompt_embd=selected_prompts)
            if self.swin.semantic_weight >= 0:
                sw = self.swin.semantic_embed_w[i](semantic_weight).unsqueeze(1)
                sb = self.swin.semantic_embed_b[i](semantic_weight).unsqueeze(1)
                x = x * self.swin.softplus(sw) + sb
    ...
    def reid_head(self,task_query,image_list,backbone_features,roi_boxes,roi_ids=None):
        features=[backbone_features[self.reid_in_feature]]
        if self.training:
            roi_boxes=[Boxes(roi_boxes[i][roi_ids[i]>-2],box_mode=BoxMode.XYXY_ABS) for i in range(len(roi_ids))]
            pos_ids=torch.cat([roi_ids[i][roi_ids[i]>-2] for i in range(len(roi_ids))],dim=0)
        ...
        for i, stage in enumerate(self.swin.stages[bonenum:]):
            task_query_stage=task_query_x.expand(-1,len(stage.blocks),-1)
            selected_prompts,p_loss=self.stage_prompts[bonenum+i](task_query_stage,f"reid{i}",train=self.training)
            prompt_loss+=p_loss
            expanded_prompts=[]
            for bi in range(len(roi_boxes)):
                expanded_prompts.append(selected_prompts[:,bi:bi+1].expand(-1,len(roi_boxes[bi]),-1,-1))
    ...
    def forward_gallery(self, image_list, gt_instances):
        ...
        pred_instances, losses, pos_match_indices = self.roi_heads(
           self.swin,self.side_stage_prompts,task_query, image_list, features, proposals, gt_instances
        )
        roi_boxes=[inst.pred_boxes.tensor for inst in pred_instances]
        if self.training:
            losses.update(proposal_losses)
            ...
            reid_loss=self.reid_head(task_query,image_list,features,roi_boxes,assign_ids)
            losses.update(reid_loss)
...
class SwinROIHeads(ROIHeads):
    ...
    def _shared_roi_transform(self, features, boxes,swin):
        ...
        x = x.permute(0, 2, 1)
        bonenum=3
        x,hw_shape = swin.side_stages[bonenum-swin.side_start_stage].downsample(x,hw_shape)
        if swin.semantic_weight >= 0:
            sw = swin.side_semantic_embed_w[bonenum-swin.side_start_stage](semantic_weight).unsqueeze(1)
            sb = swin.side_semantic_embed_b[bonenum-swin.side_start_stage](semantic_weight).unsqueeze(1)
            x = x * swin.softplus(sw) + sb
        for i, stage in enumerate(swin.side_stages[bonenum-swin.side_start_stage+1:]):
            x, hw_shape, out, out_hw_shape = stage(x, hw_shape)
            if swin.semantic_weight >= 0:
                sw = swin.side_semantic_embed_w[bonenum-swin.side_start_stage+1+i](semantic_weight).unsqueeze(1)
                sb = swin.side_semantic_embed_b[bonenum-swin.side_start_stage+1+i](semantic_weight).unsqueeze(1)
                x = x * swin.softplus(sw) + sb
...
class PromptedSwinROIHeads(SwinROIHeads):
    def _shared_roi_transform(self, features, boxes,swin,det_stage_prompts,task_query):
        ...
        for i, stage in enumerate(swin.side_stages[bonenum-swin.side_start_stage+1:]):
            task_query_stage=task_query_x.expand(-1,len(stage.blocks),-1)
            selected_prompts,p_loss=det_stage_prompts[i](task_query_stage,f"det{i}",train=self.training)
            prompt_loss+=p_loss
            expanded_prompts=[]
            for bi in range(len(boxes)):
                expanded_prompts.append(selected_prompts[:,bi:bi+1].expand(-1,len(boxes[bi]),-1,-1))
            selected_prompts=torch.cat(expanded_prompts,dim=1)
            del expanded_prompts
            x, hw_shape, out, out_hw_shape = stage(x, hw_shape,deep_prompt_embd=selected_prompts)
            if swin.semantic_weight >= 0:
    ...
    def forward(
        ...
        box_features,prompt_loss = self._shared_roi_transform(
            [features[f] for f in self.in_features], proposal_boxes,swin,det_stage_prompts,task_query
        )
        predictions = self.box_predictor(box_features.mean(dim=[2, 3]))
