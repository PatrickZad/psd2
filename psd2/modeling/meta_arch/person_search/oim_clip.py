...
@META_ARCH_REGISTRY.register()
class OimClipSimpleDetboxaugSplitAttmaskAttmixOimshareSdm(OimClipSimpleDetboxaugAttmaskOimshareSdm):
    def _contextmix(self,roi_tokens,attn):
        perm_tokens=roi_tokens[torch.randperm(roi_tokens.shape[0])]
        sort_attn_idxs=torch.argsort(attn,dim=-1)
        num_tokens=roi_tokens.shape[1]
        ...
        roi_tokens=keep_mask*roi_tokens+(1-keep_mask)*perm_tokens
        return roi_tokens
    def img_embes(self,roi_feats): # TODO
        all_tokens=roi_feats.flatten(2,3).permute(0,2,1).contiguous() # B x L x C
        attn_mask=torch.zeros((all_tokens.shape[0],all_tokens.shape[1]+1,all_tokens.shape[1]+1),dtype=all_tokens.dtype,device=all_tokens.device)
        do_mask_prob=torch.rand(all_tokens.shape[0])
        mask_prob = torch.rand((all_tokens.shape[0],all_tokens.shape[1]+1),device=all_tokens.device) # B x L
        ...
        mix_idx=rand_idx[:mix_num_samples]
        mask_idx=rand_idx[mix_num_samples:].cpu().numpy().tolist()

        for i,mp in enumerate(do_mask_prob):
            if mp<0.5 and i in mask_idx:
...
@META_ARCH_REGISTRY.register()
class OimClipSimpleDetboxaugSplitAttmaskAttmixOimshareSdmMIML2DFullyPredVe(OimClipSimpleDetboxaugAttmaskAttmixOimshareSdmMIML2DFullyPredVe):
    def img_embes(self,roi_feats): # TODO
        all_tokens=roi_feats.flatten(2,3).permute(0,2,1).contiguous() # B x L x C
        attn_mask=torch.zeros((all_tokens.shape[0],all_tokens.shape[1]+1,all_tokens.shape[1]+1),dtype=all_tokens.dtype,device=all_tokens.device)
        do_mask_prob=torch.rand(all_tokens.shape[0])
        ...
        attn_cls=attn_weights[:,0,1:]
        feats=img_feats[1:]
        embs=img_feats[0]  # feats.mean(dim=0)
        if self.training:
            return embs,feats,attn_cls.reshape(-1,roi_feats.shape[-2],roi_feats.shape[-1])
        else:
            return embs
@META_ARCH_REGISTRY.register()
class OimClipSimpleDetboxaugSplitAttmaskAttmixmidOimshareSdmMIML2DFullyPredVe(OimClipSimpleDetboxaugSplitAttmaskAttmixOimshareSdmMIML2DFullyPredVe):
    def _contextmix(self,roi_tokens,attn):
        perm_tokens=roi_tokens[torch.randperm(roi_tokens.shape[0])]
        sort_attn_idxs=torch.argsort(attn,dim=-1)
        num_tokens=roi_tokens.shape[1]
        num_mix_tokens=torch.randint(1,int(num_tokens*0.3),(roi_tokens.shape[0],))
        num_neg_tokens=((num_tokens-num_mix_tokens)/2).int()
        num_pos_tokens=num_tokens-num_mix_tokens-num_neg_tokens
        t_attn_value_low=attn[list(range(roi_tokens.shape[0])),sort_attn_idxs[list(range(roi_tokens.shape[0])),(num_neg_tokens-1).cpu().numpy().tolist()].cpu().numpy().tolist()]
        t_attn_value_high=attn[list(range(roi_tokens.shape[0])),sort_attn_idxs[list(range(roi_tokens.shape[0])),(-num_pos_tokens).cpu().numpy().tolist()].cpu().numpy().tolist()]
        keep_mask=torch.ones_like(attn)
        mix_mask=torch.logical_and(attn>t_attn_value_low.unsqueeze(1),attn<t_attn_value_high.unsqueeze(1))
        keep_mask[mix_mask]=0.
        keep_mask=keep_mask.unsqueeze(2)
        roi_tokens=keep_mask*roi_tokens+(1-keep_mask)*perm_tokens
        return roi_tokens
@META_ARCH_REGISTRY.register()
class OimClipSimpleDetboxaugSplitAttmaskAttmixhighOimshareSdmMIML2DFullyPredVe(OimClipSimpleDetboxaugSplitAttmaskAttmixOimshareSdmMIML2DFullyPredVe):
    def _contextmix(self,roi_tokens,attn):
        perm_tokens=roi_tokens[torch.randperm(roi_tokens.shape[0])]
        sort_attn_idxs=torch.argsort(attn,dim=-1)
        num_tokens=roi_tokens.shape[1]
        num_mix_tokens=torch.randint(1,int(num_tokens*0.2),(roi_tokens.shape[0],))
        t_attn_value=attn[list(range(roi_tokens.shape[0])),sort_attn_idxs[list(range(roi_tokens.shape[0])),(-num_mix_tokens).cpu().numpy().tolist()].cpu().numpy().tolist()]
        keep_mask=torch.ones_like(attn)
        keep_mask[attn>=t_attn_value.unsqueeze(1)]=0.
        keep_mask=keep_mask.unsqueeze(2)
        roi_tokens=keep_mask*roi_tokens+(1-keep_mask)*perm_tokens
